...
### Added
- **Orchestration is measured, not promised.** `claude-router stats` and the dashboard now show a **dispatch rate** (coordinator turns that called the Agent tool, over turns that were offered it), **nested dispatch** (a subagent that delegated — roles are leaves), **cost by role** (`recon`, `builder`, `gate`, … plus `coordinator`), and a **vs all-opus** counterfactual (the session's tokens priced on the opus tier; an upper bound). Every subagent and coordinator row in `history.jsonl` carries `sessionId`, `coordinator`, `dispatchable`, `dispatched`, `nested` as applicable. All optional, so existing ledgers keep folding as before.
...
### Fixed
- **Passthrough traffic shows up in the ledger.** Explicit-model `/v1/messages` requests forwarded without `--force-route` were never recorded, so the dashboard's passthrough bar was permanently zero and a user saw an empty ledger. They are now priced against the model the client named (saved = 0 by construction) on both the non-streaming and streaming paths; `count_tokens` and the non-`/v1` catch-all still never record.
- **Hop-by-hop headers are stripped on both passthrough paths** (`connection`, `transfer-encoding`, `accept-encoding`, `te`, …), through one shared forwarder instead of two hand-written copies.
- **`~/.claude-router` files are created owner-only** (`0600`; directory `0700`): the ledger, daemon state, log and config.
...
- **Stricter TypeScript** (`noUnusedLocals`, `noUnusedParameters`, `noImplicitOverride`, `noFallthroughCasesInSwitch`) instead of a linter — two dead helpers and a stray import fell out immediately. `npm run test:coverage` uses Node's built-in coverage; CI uploads the report as an artifact.
